...
async function renderLinkedin() {
  ...
  await sharp(Buffer.from(svg))
    .resize(SIZE, SIZE)
    .png()
    .toFile(path.join(OUT_DIR, 'linkedin.png'));
}

async function main() {
  fs.mkdirSync(OUT_DIR, { recursive: true });
  for (const [name, markup] of Object.entries(glyphIcons)) {
    await renderGlyph(name, markup);
    console.log(`Generated ${name}.png`);
  }
  await renderLinkedin();
  console.log('Generated linkedin.png');
}
